...
>
> 📘 公共 API 接口(`loadAligned4D`、`Aligned4DController`、参数、资源释放)请参见
> [对齐 4D MRI + 模型 API 文档](./copper3d-4D-API.md)。
...
片元着色器（`texture2d_frag.glsl`）只是采样后 ×1.5 提亮，**没有窗宽窗位**：
...
框架维护了一个数组 `preRenderCallbackFunctions`，每帧都遍历它、把里面每个函数调用一次。**「注册 tick 回调」= 把你的函数塞进这个每帧都会被执行的数组**。塞进去后，你的函数就自动获得了「每帧被调用」的能力。

```ts
addPreRenderCallbackFunction(callbackFunction) {
  this.preRenderCallbackFunctions.push(callbackFunction);  // 加进数组，返回 id
}
```
...
this.addPreRenderCallbackFunction(tick);   // 注册
